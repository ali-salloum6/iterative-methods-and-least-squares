...
int main()
{
    ...
    SquareMatrix L(A.getN()),D(A.getN()),U(A.getN());
    for (int i=0;i<A.getN();i++) {
        for (int j=0;j<A.getN();j++) {
            if (i>j) L.set(i,j,-A.get(i,j));
            if (i==j) D.set(i,j,A.get(i,j));
            if (i<j) U.set(i,j,-A.get(i,j));
        }
    }

    Matrix Dinv=inv(D);
    Matrix alpha=Dinv*(L+U);
    ColumnVector beta = Dinv*b;

    cout<<"beta:\n"<<beta;
    cout<<"alpha:\n"<<alpha;

    SquareMatrix B(A.getN()),C(A.getN());
    for (int i=0;i<A.getN();i++) {
        for (int j=0;j<A.getN();j++) {
            if (i>j) B.set(i,j,alpha.get(i,j));
            if (i<j) C.set(i,j,alpha.get(i,j));
        }
    }
    ...
    IdentityMatrix I(B.getN());
    SquareMatrix I_B = I-B;

    cout<<"I-B:\n"<<I_B;

    SquareMatrix I_Binv = inv(I_B);

    cout<<"(I-B)_-1:\n"<<I_Binv;

    ColumnVector x = I_Binv*C*beta+I_Binv*beta;
    ColumnVector diff=x-beta;

    double e = diff.get_length();
    cout<<"x(0):\n"<<beta;
    int step=0;
    for(;;) {
        step++;
